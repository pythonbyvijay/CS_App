...
# FIX: Ensure value is an integer between 0-100
progress_percentage = int(max(0, min(100, 100 - (days_left/365)*100)))
st.progress(progress_percentage, text=f"Preparation Progress: {progress_percentage}%")
...
if not st.session_state.logged_in:
    ...
    if st.session_state.role == "admin":
        menu = st.sidebar.selectbox("Admin Menu", ["Add Questions", "Upload Resources", "Manage System"])
        ...
        if menu == "Add Questions":
            with st.form("add_q", clear_on_submit=True):
                ...
                a = st.text_area("Model Answer/Code")
            ...
            if st.button("Commit Uploads"):
                ...
                st.success("Files stored in system memory!")

    # --- STUDENT VIEW (and View Mode for Admin) ---
    st.divider()
    view_mode = st.radio("Switch View:", ["Revision Bank", "Download Center"], horizontal=True)

    if view_mode == "Revision Bank":
        st.header("📖 Question Bank")
        for q in st.session_state.question_list:
            with st.expander(f"[{q['category']}] {q['question']}"):
                if "Program" in q['category']: st.code(q['answer'])
                else: st.write(q['answer'])

    elif view_mode == "Download Center":
        st.header("📥 Student Downloads")
        
        # Download Questions
        q_df = pd.DataFrame(st.session_state.question_list)
        st.download_button("📥 Download Question Bank (CSV)", q_df.to_csv(index=False), "CS_Questions.csv")
